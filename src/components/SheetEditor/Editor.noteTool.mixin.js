...
const NoteTool = {
  methods: {
     ...
     * @return {number}
     */
    getDurationFromStaveNote: function(staveNote, divisions) {
      var noteType = staveNote.getDuration();
      var numDots;

      if (staveNote.isDotted()) numDots = staveNote.dots;
      else numDots = 0;

      var index = this.NOTE_VEX_TYPES.indexOf(noteType);
      var offset = index - this.NOTE_VEX_QUARTER_INDEX;
    ...
    _calculateNoteType: function(duration, divisions) {
      var i = 0;
      var count;
      var num;
     ...
     */
    getStaveNoteTypeFromDuration: function(duration, divisions, withDots) {
      if (withDots === undefined) withDots = false;

      var result = this._calculateNoteType(duration, divisions);
      var index = this.NOTE_VEX_QUARTER_INDEX + result.index;
      var noteType = this.NOTE_VEX_TYPES[index];

      if (withDots) {
        for (var i = 0; i < result.dots; i++) noteType += "d";
      }

      return noteType;
    },

    getNoteTypeFromDuration: function(duration, divisions) {
      var result = this._calculateNoteType(duration, divisions);
      var index = this.NOTE_QUARTER_INDEX + result.index;

      return {
        type: this.NOTE_TYPES[index],
        dot: result.dots
      };
    },
    ...
    transposeNote: function(key, interval) {
      var step = key[0];
      ...
      var mod = this.TONES.length;

      var currentIndex = this.TONES.indexOf(step);
      var shifted = currentIndex + interval;
      var newIndex = shifted % mod;
      if (newIndex < 0) newIndex += mod;
      var newKey = this.TONES[newIndex];
