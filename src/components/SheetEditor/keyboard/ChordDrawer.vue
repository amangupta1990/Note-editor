<template>
  <div v-bind:class="[{ open: show }, 'drawer-container']">
    <div class="container-inner">
      <nav
        class="chord-scales"
        v-for="(grade, index) in scaleChords.grades"
        :key="index"
      >
        ...
        <div class="chordVariations">
          <button
            v-on:click="
              $emit('chordselected', {
                tonic: getChordTonic(scaleChords.chords[index]),
                variation: chordVariation
              })
            "
            v-for="(chordVariation, variationIndex) in chordVariations(
              scaleChords.chords[index]
            )"
            :key="variationIndex"
...
export default {
  ...
  computed: {
    scaleChords: function() {
      const chords =
        this.tonic === "major"
          ? Key.majorKey(this.chordNote)
          : Key.minorKey(this.chordNote);
      return chords;
    }
  }
};
